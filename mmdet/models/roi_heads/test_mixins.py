...
class BBoxTestMixin:
    ...
    def simple_test_bboxes_guigcn(self,
                           x,
                           img_metas,
                           proposals,
                           rcnn_test_cfg,
                           rescale=False):
        """Test only det bboxes without augmentation.
           self._bbox_forward(x, rois) -> self._bbox_forward(x, rois, img_metas)
        """
    ...
    def aug_test_bboxes_guigcn(self, feats, img_metas, proposal_list, rcnn_test_cfg):
        """Test det bboxes with test time augmentation.
           self._bbox_forward(x, rois) -> self._bbox_forward(x, rois, img_metas)
            """
        aug_bboxes = []
        ...
        for x, img_meta in zip(feats, img_metas):
            ...
            proposals = bbox_mapping(proposal_list[0][:, :4], img_shape,
                                     scale_factor, flip, flip_direction)
            rois = bbox2roi([proposals])
            bbox_results = self._bbox_forward(x, rois, img_meta)
            bboxes, scores = self.bbox_head.get_bboxes(
                rois,
                bbox_results['cls_score'],
                bbox_results['bbox_pred'],

    ...
    def __init__(self,
        ...
        if self.cls_gcn:
            with open(gcn_cfg['emb_path'], 'rb') as f:
                word_embeddings_info = pickle.load(f)
            if self.emb_mode == 'label':
                self.cls_word_embeddings = torch.from_numpy(word_embeddings_info['emb_label']).float()
            elif self.emb_mode == 'word':
                self.cls_word_embeddings = torch.from_numpy(word_embeddings_info['emb_word']).float()
        ...
        if self.reg_gcn:
            with open(gcn_cfg['emb_path'], 'rb') as f:
                word_embeddings_info = pickle.load(f)
            if self.emb_mode == 'label':
                self.reg_word_embeddings = torch.from_numpy(word_embeddings_info['emb_label']).float()
            elif self.emb_mode == 'word':
                self.reg_word_embeddings = torch.from_numpy(word_embeddings_info['emb_word']).float()
            elif self.emb_mode == 'random':
...
class DebugBBoxHead(GCNConvFCBBoxHead):
    '''
        Box head implementation for Graph-based relation guiding network.
    '''
    def __init__(self, *args, **kwargs):
        super(DebugBBoxHead, self).__init__(
            num_shared_fcs=2,
            *args,
            **kwargs)
